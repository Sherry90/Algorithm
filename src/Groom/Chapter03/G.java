...
class FrequencyTable
{
    ...
    void addBirthDate(int birthDate)
    {
        ...
        if(count == 0)
        { // 기존에 빈도수가 0이었다면? 새로 추가될 데이터는 unique한 데이터가 될 것이다.
            this.uniqueElements += 1;
        }else if(count == 1)
        { // 기존에 빈도수가 1이었다면? 새로 추가될 데이터로 인해 unique해지지 않을 것이다.
            this.uniqueElements -= 1;
        }
    ...
    void removeBirthDate(int birthDate)
    {
        int count = frequency[birthDate];
        if(count == 1)
        { // 기존에 빈도수가 1이었다면? 제거 될 데이터로 인해 빈도수가 0이 될 것이다..
            this.uniqueElements -= 1;
        }else if(count == 2)
        { // 기존에 빈도수가 2이었다면? 빈도수가 하나 줄어들어 unique한 데이터가 될 것이다.
            this.uniqueElements += 1;
        }
